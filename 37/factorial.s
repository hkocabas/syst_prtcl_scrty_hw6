...
_start:
	movl $0, %edi		#the index points the first element on the list
				#you can change the index number and calculate
				#other elements on the list
	movl data_items(,%edi,4), %eax	#get first byte of the element form fata_items
	movl %eax, %ebx		#assign the %eax address to the %ebx address
	pushl %ebx 		#The factorial takes one argument - the
